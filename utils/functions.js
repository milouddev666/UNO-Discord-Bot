...
module.exports = {
	...
	renderHandBuffer: function (cards, guildID, playableSet) {
		return new Promise(async (resolve, reject) => {
			var maxCards = config.cardsImageMax || 20;
			var count = Math.min(cards.length, maxCards);
			var inputs = [];

			for (var i = 0; i < count; i++) {
				var card = cards[i];
				inputs.push({
					input: "./assets/images/defaultCards/" + card + ".png",
					left: 95 * i,
					top: 0
				});
				// Playability indicator bar
				var isPlayable = playableSet ? playableSet.has(card) : true;
				var barColor = isPlayable
					? { r: 0, g: 200, b: 0, alpha: 0.85 }
					: { r: 160, g: 160, b: 160, alpha: 0.85 };
				inputs.push({
					input: { create: { width: 95, height: 5, channels: 4, background: barColor } },
					left: 95 * i,
					top: 0
				});
			}
	...
	endGame: async function (guild, gameData) {
		return new Promise(async (resolve, reject) => {
			...
			if (existsSync(`./assets/images/serverCards/${guild.id}_front.png`)) {
				//  Delete cards from this server
				unlink(`./assets/images/serverCards/${guild.id}_front.png`, (err) => {
					if (err) {
						console.error(err);
						return;
					}
	...
	formatGameSetting: async function (setting) {
		...
		else if (["startingcards", "startc"].includes(setting)) return "StartingCards";
			...
			async function sendEmbed(address) {
				return new Promise(async (resolve, reject) => {
					...
					if (address.dmEmbed && (userOptions.DMCards || gameData.gameSettings.UseOneChannel)) {
						// Block ALL DM delivery when Interaction UI is active
						if (config.useInteractionUI) {
							console.log("[InteractionUI] DM blocked for " + (playerID || "?"));
							return resolve();
						}
						...
						if (!address.dmEmbed && !address.default && gameData.gameSettings.UseOneChannel) {
							...
							// In Interaction UI mode with an existing board, skip creating new messages
							if (config.useInteractionUI && gameData && gameData.boardMessageID) {
	...
	// Finds the amount of docs where total wins are void
		var nullWinsDocs = await userModel.find({ "stats.totalWins": null });
		docCount = docCount - nullWinsDocs.length;
