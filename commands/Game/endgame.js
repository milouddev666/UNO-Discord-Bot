...
class Endgame extends Command {
	...
	async run(client, msg, { prefix, gameData }) {
		...
		if (gameData) {
			...
			// Delete board message if interaction UI
			if (config.useInteractionUI && gameData.boardMessageID && gameData.boardChannelID) {
				try {
					var boardChan = msg.guild.channels.get(gameData.boardChannelID);
					if (boardChan) {
						var boardMsg = await boardChan.getMessage(gameData.boardMessageID);
