...
class Leavegame extends Command {
	...
	async run(client, msg, { prefix, gameData }) {
		...
		if (gameData.status == "preGame") {
			...
			var deletedChannelID = gameData.players[msg.author.id].channelID;
			...
			if (gameData.currentPosition > gameData.playerOrder.indexOf(msg.author.id)) {
			...
			// Update board via the unified refresh system
