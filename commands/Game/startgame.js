...
const { topggVoteURL, gameSettings: configGameSettings } = config;
const { msgReactions, currentGames } = require("../../utils/collections");
const { buildActionRow, buildInteractionEmbed } = require("../../helpers/components");
const { renderBoardImage } = require("../../helpers/boardRenderer");
...
class Startgame extends Command {
	...
	async run(client, msg, { guildData, gameData, args, prefix }) {
		...
		// Force single-channel mode when interaction UI is enabled
		if (config.useInteractionUI) {
			gameSettings.UseOneChannel = true;
			customGameSettings.UseOneChannel = true;
		}
		...
		if (gameSettings.UseOneChannel) {
			...
			// Suppress legacy greeting in single-channel mode when Interaction UI is active
			if (!config.useInteractionUI) {
		...
		// Create persistent board message if interaction UI is enabled
		if (config.useInteractionUI) {
			console.log("[InteractionUI] Board creation started");
			gameData.status = "midGame";
			try {
				var boardChannelID = msg.channel.id;
				var boardChannel = msg.channel.guild.channels.get(boardChannelID);
				if (!boardChannel) {
					console.log("[InteractionUI] Board creation failed: boardChannel is null");
					try { await msg.channel.createMessage("[InteractionUI] ERROR: Board channel not found (this should never happen)"); } catch (_) {}
				} else {
				...
				var boardEmbed = await buildInteractionEmbed(gameData, language);
				if (!boardImageBuf) {
					delete boardEmbed.image;
				}
				var boardRow = await buildActionRow(gameData.playerOrder[gameData.currentPosition], gameData, language);
					...
					var boardMsg = await boardChannel.createMessage(createOpts);
					console.log("[InteractionUI] Board creation success (id: " + boardMsg.id + ")");
				...
				console.log("[InteractionUI] Board creation failed:", e);
				if (e.stack) console.log(e.stack);
				try {
					await msg.channel.createMessage("[InteractionUI] Board creation failed:\n```\n" + (e.stack || e.message) + "\n```");
				} catch (_) {}
			}
		}
