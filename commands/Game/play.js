...
const { updateInteractionBoard } = require("../../helpers/components");
...
class Play extends Command {
	...
	async run(client, msg, { gameData, args, prefix }) {
		...
		if (config.useInteractionUI && gameData.boardMessageID && msg.author.id !== client.user.id) {
			return msg.error(await translate("game.text.play.useButtons", language), { bold: false });
		}

		if (!gameData.channelIDs.includes(msg.channel.id)) return msg.error(await translate("game.play.error.noUnoChannel", language));
		...
		if (gameData.players[oldPlayerID].cards.length > 0) {
			...
			await updateInteractionBoard(gameData, language, msg.guild, client);
		} else {
			await gameData.updateOne({
				status: "postGame",
				currentCard: gameData.currentCard,
